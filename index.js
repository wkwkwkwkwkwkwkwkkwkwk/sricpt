...
const { praymenu } = require('./src/praymenu')
...
const vcard = 'BEGIN:\n' //
            + 'VERSION:3.0\n' //
            + 'FN: OWNER GANS\n' //Nama untuk kontak lu
            + 'ORG:Creator YIYI;\n' //Nama kayak desk
            + 'TEL;type=CELL;type=VOICE;waid=6285752164908:+62-857-5216-4908\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `__________\n\n\n\n\n\n----------\n-----\nBEBAN GROUP \n_*${mdata.subject}*_ MENAMBAH\n*_____________*\n@${num.split('@')[0]}\nSELAMAT BERGABUNG\n *_____________*\nMoga betah Di group Dan Jangan Lupa Intro Yha`
				...
				teks = `hey kak @${num.split('@')[0]} Yiyi Titip Gorengan Yah Btw Gausa Kembali Lagi 😋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[TUNGGU] YIYI sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				asik: '[TUNGGU] YIYI sedang Memperoses🍥Orang sabar Di Sayang Dajal!',
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD YIYI BOT*',
					benned: 'Anda Ke Band Silahkan Hubungi Owner Agar Membuka Band Anda',
					ownerG: '[❗] Lu spe owner group? ini fitur khusus owner group njierrr ❌',
					ownerB: '[❗] Lu spe owner bot? ini fitur khusus owner bot njierrr ❌',
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman Yiyi Daftar Untuk Berteman Dengan Yiyi bot dengan cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar yiyi|17\n\n──「 YIYI BOT 」──`,
			...
			const ownerNumber = ["6285752164908@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6282232050382@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = [ownerNumber,"6282232050382@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["6282232050382@s.whatsapp.net"]//ubah nomor lo
			const premium = ["6282232050382@s.whatsapp.net","6285248328218@s.whatsapp.net","6283147914634@s.whatsapp.net"]
       ...
       //function leveling
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, praymenu(prefix), text, { quoted: mek })
					break
					case 'ownermenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*YIYI GROUP*\n\nLink : https://chat.whatsapp.com/JFsC1TBRlbzIOtGp7phkrj', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
                   ...
                   client.sendMessage(from, modapk(prefix), text, { quoted: mek })
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('Fitur Nsfw Belum Dihidupkan Ketik /nsfw On jika ingin mematikan ketik /nsfw Off')
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT YIYI ✨* 」\n*│+wa.me/6285752164908*╰──────*「 *YIYI* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  YIYI BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6285752164908*╰──────*「 * YIYI* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  YIYI BOT_*\n*_Ketik /daftarvip*' })
					break
					case 'cekmod': 
                ...
                break
                    case 'modbotlist':
					teks = 'This is list of user modbot :\n'
					for (let p of mod) {
						teks += `~> @${p.split('@')[0]}\n`
					}
	...
	break
					...
					if (body.startsWith("iri"))
conn.sendMessage(m.key, 'blabla', MessageType.audio, {mimetype: "audio/mp4", ptt: true})
                case 'irii':
...
if (isBanned) return reply(mess.only.benned)    
	...
	if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					const dare =['Kirim pesan ke mantan kamu dan bilang "aku masih suka sama kamu','telfon crush/pacar sekarang dan ss ke pemain','pap ke salah satu anggota grup','Bilang "KAMU CANTIK BANGET NGGAK BOHONG" ke cowo','ss recent call whatsapp','drop emot "🦄💨" setiap ngetik di gc/pc selama 1 hari','kirim voice note bilang can i call u baby?','drop kutipan lagu/quote, terus tag member yang cocok buat kutipan itu','pake foto sule sampe 3 hari','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " anjimm gabutt anjimmm " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll','Kirim sms pada orangtuamu "Hai, bro! Aku baru beli majalah Playboy edisi terbaru!"',                                                        'Kirim sms pada orangtuamu, "Ma, Pa, aku sudah tahu bahwa aku adalah anak adopsi dari Panti Asuhan. Jangan menyembunyikan hal ini lagi."',
...
'Marah-marah ketemen kamu yang gak dateng padahal udah janjian mau main "truth or dare" bareng"',
...
'Menyanyikan lagu Iwak Peyek dengan keras di rumah.',
...
'Nyebutin 1 biru sampai 10 biru dengan cepat dan tidak boleh melakukan kesalahan. Jika salah maka harus diulang dari awal (sambil vn).',
'Pakai mahkota tiruan dari kertas buku dan bilang sama setiap orang di ruangan SAYA ADALAH RAJA/RATU AMBILKAN SAYA MINUM']
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6285752164908@s.whatsapp.net', options, text, {quoted: mek})
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/YIYIjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`YIYIJOKES\`\`\`')
				break
					case 'kata':
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  YIYI BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buffer, image, {caption: `[ Yiyi Broadcast ]\n\n${body.slice(4)}`})
						}
						reply('Suksess broadcast')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ * YIYI Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner YIYI!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin YIYI!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
